...
function Start(){
    ...
    const handleValidate = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/validate-code`, { code: fullCode });

            if (response.status === 200 && response.data.operatorToken) {
                const { operatorToken } = response.data;
                
                // Store the token in localStorage
                localStorage.setItem('operatorToken', operatorToken);
                
                // Store the token in cookies
                document.cookie = `operatorToken=${operatorToken}; path=/; max-age=86400`; // Set cookie for 24 hours
